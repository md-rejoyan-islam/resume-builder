...
import { useCreateDisclosureLetterMutation } from "@/lib/features/disclosure-letter/disclosure-letter-slice";
import { emptyDisclosureLetterData } from "@/lib/features/disclosure-letter/disclosure-letter-format";
import {
  ChevronRight,
  FileText,
  Loader2,
...
import { DisclosureLetterPdfUploadDialog } from "./pdf-upload-dialog";

export function DisclosureLetterSelectionCards() {
  const router = useRouter();
  const [createDisclosureLetter, { isLoading: isCreating }] =
    useCreateDisclosureLetterMutation();
  const [isPdfDialogOpen, setIsPdfDialogOpen] = useState(false);

  const handleBuildFromScratch = async () => {
    try {
      // Create a new disclosure letter with empty data in the backend
      const result = await createDisclosureLetter({
        title: "Untitled Disclosure Letter",
        status: "draft",
        disclosureType: emptyDisclosureLetterData.disclosureType,
        personalInfo: emptyDisclosureLetterData.personalInfo,
        disclosureContent: emptyDisclosureLetterData.disclosureContent,
      }).unwrap();

      // Navigate to the disclosure letter builder with the MongoDB ID
      router.push(`/disclosure-letters/${result.data._id}/new`);
    } catch (error) {
      console.error("Failed to create disclosure letter:", error);
    }
  };
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <CardContent className="relative space-y-5 p-0">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm sm:text-base text-muted-foreground leading-relaxed">
                Create a fully customized disclosure letter with complete
        ...
        <Card className="relative h-full p-6 sm:p-8 bg-card/40 dark:border-accent/10 border-accent/30 hover:dark:border-accent/20 rounded-2xl hover:border-accent/50 hover:shadow-xl shadow-none hover:shadow-accent/10 transition-all duration-300 cursor-pointer overflow-hidden">
          ...
          <CardContent className="relative space-y-5 p-0">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm sm:text-base text-muted-foreground leading-relaxed">
                Upload your existing PDF disclosure letter to import your
      ...
      {/* PDF Upload Dialog */}
      <DisclosureLetterPdfUploadDialog
        ...
        <Card className="relative h-full p-6 sm:p-8 bg-card/40 border-2 border-dashed dark:border-accent/10 border-accent/30 hover:dark:border-accent/20 rounded-2xl shadow-none overflow-hidden">
          ...
          <CardContent className="relative space-y-5 opacity-60 p-0">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm sm:text-base text-muted-foreground leading-relaxed">
                Generate a professional disclosure letter instantly using AI
