...
export function ResumeSidebar({ steps, currentStep, onStepChange }: ResumeSidebarProps) {
  return (
    <aside className="hidden lg:flex w-64 bg-white dark:bg-card border-r border-border flex-col h-full z-10 shadow-[4px_0_24px_-12px_rgba(0,0,0,0.1)]">
            ...
            return (
              <li key={step.id}>
                <button
                  ...
                  className={cn(
                    "w-full flex items-center gap-3 px-4 py-2 cursor-pointer rounded-lg text-sm font-medium transition-all duration-200",
                    isActive
                      ? "bg-blue-50 text-primary dark:bg-primary/10  "
                      : "text-slate-600 dark:text-muted-foreground hover:bg-slate-50 dark:hover:bg-accent/10 hover:text-slate-900 dark:hover:text-foreground"
                  )}
                >
                  <div className={cn(
      ...
      <div className="p-4 border-t border-border space-y-4">
        {/* New Preview Button */}
        <Button variant="outline" className="w-full justify-center gap-2 text-primary border-primary/20 hover:bg-primary/5 hover:border-primary/40 transition-all">
        ...
        <div className="bg-slate-50 dark:bg-accent/10 rounded-xl p-4 flex items-center gap-4 border border-slate-100 dark:border-border">
          ...
          <div>
            <span className="text-xs font-semibold text-muted-foreground block">Profile Completion</span>
