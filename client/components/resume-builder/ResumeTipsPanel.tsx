...
export function ResumeTipsPanel({ title, tips, didYouKnow }: ResumeTipsPanelProps) {
  return (
    <div className="lg:col-span-1">
      <div className="bg-white dark:bg-card rounded-xl border border-border p-6 sticky top-8">
          ...
          {tips.map((tip, index) => (
            <div key={index} className="flex gap-3 items-start">
              ...
              <p className="text-sm text-slate-600 dark:text-muted-foreground leading-relaxed">
                <span className="font-medium text-slate-900 dark:text-foreground">{tip.title}:</span> {tip.text}
              </p>
            </div>
          ))}
        </div>
        ...
        {didYouKnow && (
          <div className="mt-8 p-4 bg-blue-50 dark:bg-primary/5 rounded-lg border border-blue-100 dark:border-primary/10">
            <h4 className="text-xs font-bold text-primary uppercase tracking-wider mb-2">Did you know?</h4>
