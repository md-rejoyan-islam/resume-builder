...
import { useUploadPdfResumeMutation } from "@/lib/features/resume/resume-slice";
...
export function PdfUploadDialog({ open, onOpenChange }: PdfUploadDialogProps) {
  ...
  const [uploadPdfResume, { isLoading: isUploading }] =
  ...
  const handleFileSelect = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        setProcessingStep("Extracting resume data with AI...");

        // Call the API
        const result = await uploadPdfResume(formData).unwrap();

        setProcessingStep("Resume created successfully!");
        setUploadSuccess(true);

        // Navigate to the resume builder after a short delay
        setTimeout(() => {
          onOpenChange(false);
    ...
    [uploadPdfResume, onOpenChange, router]
  );
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload your existing resume PDF. We&apos;ll use AI to extract your
