import { DisclosureLetterSelectionCards } from "@/components/disclosure-letters/disclosure-letter-selection-cards";
import { Card, CardContent } from "@/components/ui/card";
import { FileText, Scale, Sparkles } from "lucide-react";
import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "Create New Disclosure Letter | Professional Disclosure Letter Builder",
  description:
    "Create professional disclosure letters from scratch, use existing templates, or generate with AI. Build your disclosure letter in minutes.",
};

export default function DisclosureLetterCreatePage() {
  return (
    <div className="min-h-screen space-y-12 pb-12">
      ...
      <div className="space-y-4">
        <div className="inline-flex items-center gap-2 px-4 py-2 bg-primary/10 text-primary rounded-full text-sm font-medium">
          ...
          Create Professional Disclosure Letters
        </div>
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-foreground">
          Choose Your Method
        </h1>
        <p className="text-base sm:text-lg text-muted-foreground max-w-2xl">
          Select how you&apos;d like to create your disclosure letter and get
          started in seconds
        </p>
      </div>

      {/* Cards Grid */}
      <DisclosureLetterSelectionCards />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <Card className="group p-5 sm:p-6 bg-card/40 dark:border-accent/10 border-accent/30 hover:dark:border-accent/20 rounded-xl hover:border-accent/50 hover:shadow-lg transition-all duration-300 shadow-none">
          <CardContent className="flex items-start gap-4 p-0">
            <div className="p-2.5 bg-primary/10 rounded-lg group-hover:bg-primary group-hover:scale-110 transition-all">
              <Scale className="w-5 h-5 text-accent group-hover:text-accent-foreground" />
            </div>
            <div className="flex-1 space-y-1">
